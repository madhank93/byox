...
func findTableRootPage(schemaRows []record, tableName string) int {
	for _, r := range schemaRows {
		if string(r.values[2]) == tableName {
			return int(decodeInt(r.values[3], r.serialTypes[3]))
		}
	}
...
func main() {
	...
	case strings.HasPrefix(strings.ToUpper(command), "SELECT COUNT(*)"):
		...
		schemaRows := readSchemaRecords(databaseFile, pageSize)
		rootPage := findTableRootPage(schemaRows, tableName)
