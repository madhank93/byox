	...
	for _, t := range topics {
		...
		for _, p := range t.partitions {
			...
			if topicKnown && partitionExists(topic, p.id) {
				...
				baseOffset = 0
				logStartOffset = 0
				writePartitionLog(t.name, p.id, p.recordBatches)
			}
...
func readPartitionLog(topicName string, partition int32) []byte {
	data, err := os.ReadFile(partitionLogPath(topicName, partition))
	if err != nil {
		return nil
	}
	return data
...
// writePartitionLog appends data (one or more already-encoded record
// batches, verbatim from a Produce request) to a topic-partition's log
// segment file, creating the partition's directory and file if needed.
func writePartitionLog(topicName string, partition int32, data []byte) error {
	dir := fmt.Sprintf("/tmp/kraft-combined-logs/%s-%d", topicName, partition)
	if err := os.MkdirAll(dir, 0755); err != nil {
		return err
	}
	f, err := os.OpenFile(partitionLogPath(topicName, partition), os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		return err
	}
	defer f.Close()
...
func partitionLogPath(topicName string, partition int32) string {
	return fmt.Sprintf("/tmp/kraft-combined-logs/%s-%d/00000000000000000000.log", topicName, partition)
}
