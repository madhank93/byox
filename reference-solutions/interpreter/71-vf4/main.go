...
// ClassStmt is "class name { }".
type ClassStmt struct {
	Name string
	Line int
}

// LoxClass is a class value: printable and, later, callable to construct
// instances.
...
func (p *Parser) parseClassDecl() (Stmt, error) {
	...
	return ClassStmt{Name: nameTok.Lexeme, Line: nameTok.Line}, nil
}
...
func execute(stmt Stmt, env *Environment) error {
	...
	case FunctionStmt:
		...
		return nil
	case ClassStmt:
		env.Define(s.Name, &LoxClass{Name: s.Name})
		return nil
	case ReturnStmt:
		var value interface{}
		if s.Value != nil {
