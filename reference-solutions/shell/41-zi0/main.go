...
func runCompleter(path, command, word, prevWord string) []string {
	out, err := exec.Command(path, command, word, prevWord).Output()
	if err != nil {
		return nil
	}
		...
		case '\t':
			...
			if lastSpace == -1 {
				...
				if path, ok := completers[commandName]; ok {
					matches = runCompleter(path, commandName, word, prevWord)
				} else {
