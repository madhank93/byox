...
func replicateFrom(target string) {
	...
	for {
		...
		// GETACK asks how much of the stream this replica has processed.
		// Nothing has been counted yet, so the answer is 0 — which is also
		// the right answer while no commands have been received.
		if len(args) >= 2 && strings.EqualFold(args[0], "REPLCONF") && strings.EqualFold(args[1], "GETACK") {
			conn.Write(encodeCommand([]string{"REPLCONF", "ACK", "0"}))
			continue
		}
		execute(io.Discard, args)
	}
