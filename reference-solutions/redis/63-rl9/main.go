...
func dispatch(c *client, args []string) {
	...
	case "EXEC":
		...
		// Each queued command's normal reply becomes one element of the
		// array, so the handlers write into a buffer rather than the socket.
		// An error abandons the batch — which is the intuitive reading of a
		// transaction, and the one stage 64 corrects.
		var body bytes.Buffer
		for _, qargs := range batch {
			var one bytes.Buffer
			execute(&one, qargs)
			if bytes.HasPrefix(one.Bytes(), []byte("-")) {
				c.w.Write(one.Bytes())
				return
			}
			body.Write(one.Bytes())
		}
	...
	default:
		...
		if c.inMulti {
			queued = append(queued, append([]string(nil), args...))
...
// queued holds the commands waiting for EXEC.
var queued [][]string
