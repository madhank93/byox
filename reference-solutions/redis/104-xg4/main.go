...
// geoPositions remembers the coordinates a location was added with.
var geoPositions = map[string]map[string][2]float64{}
...
func execute(w io.Writer, args []string) {
	{
		...
		case "GEOADD":
			if len(args) > 4 {
				...
				if geoPositions[args[1]] == nil {
					geoPositions[args[1]] = map[string][2]float64{}
				}
				geoPositions[args[1]][member] = [2]float64{lon, lat}
				bumpVersion(args[1])
				mu.Unlock()
				if existed {
					conn.Write([]byte(":0\r\n"))
		...
		case "GEOPOS":
			if len(args) > 1 {
				mu.Lock()
				set := zsets[args[1]]
				// One element per member asked for: a member that is not in
				// the set is a null array rather than a gap, so the reply
				...
				for _, member := range args[2:] {
					...
					// The coordinates are kept alongside the score for now.
					// Stage 105 drops this map and reads them back out of the
					// score itself.
					pos := geoPositions[args[1]][member]
					parts = append(parts, arrayOf(
						bulkString(strconv.FormatFloat(pos[0], 'f', 17, 64)),
						bulkString(strconv.FormatFloat(pos[1], 'f', 17, 64)),
					))
				}
				mu.Unlock()
				conn.Write(arrayOf(parts...))
