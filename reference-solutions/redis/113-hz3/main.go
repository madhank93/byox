...
func handleConn(conn net.Conn) {
	...
	cs := &clientState{conn: conn, authed: defaultNoPass()}
...
func handleCommand(cs *clientState, name string, args []string) []byte {
	// Unauthenticated connections may only authenticate.
	if !cs.authed && name != "AUTH" {
		return simpleErr("NOAUTH Authentication required.")
	}
	if name == "AUTH" {
		return cmdAuth(cs, args)
	}
	if name == "ACL" {
		return cmdAcl(cs, args)
	}
...
// --- geospatial (52-bit geohash over a sorted set) ---
...
func geoEncode(lon, lat float64) uint64 {
	latOff := (lat - geoLatMin) / (geoLatMax - geoLatMin) * float64(uint64(1)<<geoStep)
	lonOff := (lon - geoLonMin) / (geoLonMax - geoLonMin) * float64(uint64(1)<<geoStep)
	return interleave(uint32(latOff), uint32(lonOff))
}

func geoDecode(score uint64) (lon, lat float64) {
	ilat := compact(score)
	ilon := compact(score >> 1)
	latMin := geoLatMin + float64(ilat)/float64(uint64(1)<<geoStep)*(geoLatMax-geoLatMin)
	latMax := geoLatMin + float64(ilat+1)/float64(uint64(1)<<geoStep)*(geoLatMax-geoLatMin)
	lonMin := geoLonMin + float64(ilon)/float64(uint64(1)<<geoStep)*(geoLonMax-geoLonMin)
	lonMax := geoLonMin + float64(ilon+1)/float64(uint64(1)<<geoStep)*(geoLonMax-geoLonMin)
...
func cmdGeoAdd(args []string) []byte {
	if len(args) < 5 {
		return errResp("wrong number of arguments for 'geoadd'")
	}
	key := args[1]
	zsetsMu.Lock()
	defer zsetsMu.Unlock()
	...
	for i := 2; i+2 < len(args); i += 3 {
		...
		if err1 != nil || err2 != nil || lon < geoLonMin || lon > geoLonMax || lat < geoLatMin || lat > geoLatMax {
			return errResp(fmt.Sprintf("invalid longitude,latitude pair %.6f,%.6f", lon, lat))
		}
		member := args[i+2]
		if _, ok := zsets[key][member]; !ok {
			added++
		}
		zsets[key][member] = float64(geoEncode(lon, lat))
	}
	return []byte(fmt.Sprintf(":%d\r\n", added))
}
...
func cmdGeoPos(args []string) []byte {
	if len(args) < 2 {
		return errResp("wrong number of arguments for 'geopos'")
	}
	zsetsMu.Lock()
	defer zsetsMu.Unlock()
	var out [][]byte
	for _, m := range args[2:] {
		score, ok := zsets[args[1]][m]
		if !ok {
			out = append(out, nullArray())
			continue
		}
		lon, lat := geoDecode(uint64(score))
		out = append(out, arrayOf(
...
func cmdGeoDist(args []string) []byte {
	if len(args) < 4 {
		return errResp("wrong number of arguments for 'geodist'")
	}
	zsetsMu.Lock()
	s1, ok1 := zsets[args[1]][args[2]]
	s2, ok2 := zsets[args[1]][args[3]]
	zsetsMu.Unlock()
	if !ok1 || !ok2 {
		return nullBulk()
	}
	...
	return bulkString(strconv.FormatFloat(haversine(lon1, lat1, lon2, lat2), 'f', 4, 64))
}

func cmdGeoSearch(args []string) []byte {
	// GEOSEARCH key FROMLONLAT <lon> <lat> BYRADIUS <radius> <unit>
	if len(args) < 8 {
		return errResp("wrong number of arguments for 'geosearch'")
	}
	key := args[1]
	lon, _ := strconv.ParseFloat(args[3], 64)
	lat, _ := strconv.ParseFloat(args[4], 64)
	radius, _ := strconv.ParseFloat(args[6], 64)
	if strings.ToLower(args[7]) == "km" {
		radius *= 1000
	}

	zsetsMu.Lock()
	members := sortedMembers(key)
	z := zsets[key]
	zsetsMu.Unlock()

	var out [][]byte
	for _, m := range members {
		mlon, mlat := geoDecode(uint64(z[m]))
		if haversine(lon, lat, mlon, mlat) <= radius {
			out = append(out, bulkString(m))
		}
	}
	return arrayOf(out...)
}
...
type aclUser struct {
	nopass    bool
	passwords []string // SHA-256 hex hashes
}

var (
	aclMu       sync.Mutex
	defaultUser = &aclUser{nopass: true}
)
...
func cmdAcl(cs *clientState, args []string) []byte {
	if len(args) < 2 {
		return errResp("wrong number of arguments for 'acl'")
	}
	...
	case "GETUSER":
		...
		if defaultUser.nopass {
			flags = append(flags, bulkString("nopass"))
		}
		var pw [][]byte
		for _, h := range defaultUser.passwords {
			pw = append(pw, bulkString(h))
		}
		return arrayOf(bulkString("flags"), arrayOf(flags...), bulkString("passwords"), arrayOf(pw...))
	case "SETUSER":
		aclMu.Lock()
		defer aclMu.Unlock()
		for _, rule := range args[3:] {
			...
			case strings.HasPrefix(rule, ">"):
				...
				defaultUser.passwords = append(defaultUser.passwords, sha256hex(rule[1:]))
			case rule == "nopass":
				defaultUser.nopass = true
				defaultUser.passwords = nil
			}
		}
		return []byte("+OK\r\n")
	default:
		return errResp("unknown ACL subcommand")
	}
}

func cmdAuth(cs *clientState, args []string) []byte {
	var password string
	switch len(args) {
	case 2:
		password = args[1]
	case 3:
		password = args[2]
	default:
		return errResp("wrong number of arguments for 'auth'")
	}
	aclMu.Lock()
	ok := defaultUser.nopass
	for _, h := range defaultUser.passwords {
		if h == sha256hex(password) {
			ok = true
		}
	}
	aclMu.Unlock()
	if !ok {
		return simpleErr("WRONGPASS invalid username-password pair or user is disabled.")
	}
	cs.authed = true
	return []byte("+OK\r\n")
}

func defaultNoPass() bool {
	aclMu.Lock()
	defer aclMu.Unlock()
