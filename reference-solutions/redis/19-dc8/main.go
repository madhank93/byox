...
func setupAOF() {
	...
	// Open in append mode: truncating on open would destroy exactly the data
	// the AOF exists to protect.
	...
	// The manifest names the files that make up the AOF, so recovery is
	// deterministic instead of depending on what a directory listing returns.
