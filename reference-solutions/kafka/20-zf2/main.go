...
func produceBody(r *byteReader) []byte {
	...
	for _, t := range topics {
		...
		for _, p := range t.partitions {
			...
			body = binary.BigEndian.AppendUint64(body, uint64(minusOne)) // base_offset
			body = binary.BigEndian.AppendUint64(body, uint64(minusOne)) // log_append_time_ms
			body = binary.BigEndian.AppendUint64(body, uint64(minusOne)) // log_start_offset
