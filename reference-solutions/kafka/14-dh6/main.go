...
// fetchBody builds the Fetch (v16) response body. For this stage, only
// the empty-topics case is handled: throttle_time_ms/error_code/session_id
// are all 0, and the responses array echoes back however many topics
// were requested (0 for now).
func fetchBody(r *byteReader) []byte {
	...
	body = appendUvarint(body, uint64(topicCount+1))
	body = append(body, 0) // TAG_BUFFER (empty)
	return body
}
...
// returning topic metadata keyed by topic name. A missing or unreadable
// log file yields an empty map, so every topic reports as unknown rather
// than crashing the broker.
func loadClusterMetadata() map[string]*topicInfo {
